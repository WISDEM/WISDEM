...
class CylinderFrame3DD(ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        # rigid base
        node = discrete_inputs['kidx'] + np.ones(len(discrete_inputs['kidx']))   # add one because 0-based index but 1-based node numbering
        rigid = RIGID
        ...
        N = discrete_inputs['midx'] + np.ones(len(discrete_inputs['midx']))
        ...
        # point loads
        nF = discrete_inputs['plidx'] + np.ones(len(discrete_inputs['plidx']))
