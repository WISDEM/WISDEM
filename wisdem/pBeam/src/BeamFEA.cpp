...
namespace BeamFEA {
  ...
  void addTipMassContribution(const TipData &tip, int nodes, Matrix &M, Vector &F){
    ...
    std::vector<int> r(DOF);
    for (int k=0; k<DOF; k++) r[k] = (nodes-1)*DOF + k;
  ...
  int applyBaseBoundaryCondition(const double Kbase[], const bool rigidDirections[], int nodes, 
    ...
    K.resize(length, length);
    M.resize(length, length);
    Nother.resize(length, length);
    N.resize(length, length);
    F.resize(length);
	...
	K(ii,jj) = KFull(idx[ii], idx[jj]);
	M(ii,jj) = MFull(idx[ii], idx[jj]);
	N(ii,jj) = NFull(idx[ii], idx[jj]);
	Nother(ii,jj) = NotherFull(idx[ii], idx[jj]);
      }
    }
