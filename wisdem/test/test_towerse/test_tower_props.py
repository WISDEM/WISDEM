...
class TestProps(unittest.TestCase):
    ...
    def testDiscYAML_Land_1Material(self):
        ...
        self.inputs["sigma_ult_mat"] = 1e7 * np.ones((1, 3))
        self.inputs["wohler_exp_mat"] = np.array([1e1])
        self.inputs["wohler_A_mat"] = np.array([1e1])
        ...
        npt.assert_equal(self.outputs["sigma_ult"], 1e7 * np.ones(4))
        npt.assert_equal(self.outputs["wohler_exp"], 1e1 * np.ones(4))
        npt.assert_equal(self.outputs["wohler_A"], 1e1 * np.ones(4))
    ...
    def testDiscYAML_Land_2Materials(self):
        ...
        self.inputs["sigma_ult_mat"] = 1e7 * np.vstack((np.ones((1, 3)), 2 * np.ones((1, 3))))
        self.inputs["wohler_exp_mat"] = np.array([1e1, 2e1])
        self.inputs["wohler_A_mat"] = np.array([1e1, 2e1])
        ...
        npt.assert_almost_equal(self.outputs["sigma_ult"], 1e7 * np.array([1, 1, xx2, 2]))
        npt.assert_almost_equal(self.outputs["rho"], 1e4 * np.array([1, 1, xx1, 2]))
        npt.assert_almost_equal(self.outputs["wohler_exp"], 1e1 * np.array([1, 1, xx2, 2]))
        npt.assert_almost_equal(self.outputs["wohler_A"], 1e1 * np.array([1, 1, xx2, 2]))
    ...
    def testDiscYAML_Monopile_1Material(self):
        ...
        npt.assert_equal(self.outputs["sigma_ult"], 1e7 * np.ones(8))
        npt.assert_equal(self.outputs["wohler_exp"], 1e1 * np.ones(8))
        npt.assert_equal(self.outputs["wohler_A"], 1e1 * np.ones(8))
    ...
    def testDiscYAML_Monopile_RedoPileNodes(self):
        ...
        npt.assert_equal(self.outputs["sigma_ult"], 1e7 * np.ones(23))
        npt.assert_equal(self.outputs["wohler_exp"], 1e1 * np.ones(23))
        npt.assert_equal(self.outputs["wohler_A"], 1e1 * np.ones(23))
    ...
    def testDiscYAML_Monopile_DifferentMaterials(self):
        ...
        npt.assert_equal(self.outputs["sigma_ult"], 1e7 * np.r_[2 * np.ones(4), np.ones(4)])
        npt.assert_equal(self.outputs["wohler_exp"], 1e1 * np.r_[2 * np.ones(4), np.ones(4)])
        npt.assert_equal(self.outputs["wohler_A"], 1e1 * np.r_[2 * np.ones(4), np.ones(4)])
    ...
    def testTowerDisc(self):
        ...
        # npt.assert_equal(self.outputs["sigma_ult_full"], self.inputs["sigma_ult"][0] * np.ones(6))
        ...
        npt.assert_equal(self.outputs["foreaft_iner"], 1e3 * 11 * np.ones(nout) * of)
        npt.assert_equal(self.outputs["sideside_iner"], 1e3 * 11 * np.ones(nout) * of)
        npt.assert_equal(self.outputs["foreaft_stff"], 6 * 11 * np.ones(nout) * of)
        npt.assert_equal(self.outputs["sideside_stff"], 6 * 11 * np.ones(nout) * of)
        npt.assert_equal(self.outputs["tor_stff"], 7 * 10 * np.ones(nout) * of)
        npt.assert_equal(self.outputs["axial_stff"], 6 * 9 * np.ones(nout) * of)
