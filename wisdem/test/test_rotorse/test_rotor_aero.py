...
class TestRotorAero(unittest.TestCase):
    ...
    def testRegulationTrajectory(self):
        ...
        npt.assert_almost_equal(self.outputs['Omega'], Omega_tsr)
        npt.assert_equal(self.outputs['pitch'], np.zeros( V_expect0.shape ) )
        npt.assert_equal(self.outputs['Cp'], self.outputs['Cp_aero']*0.95)
        npt.assert_almost_equal(self.outputs['Cp'], self.outputs['Cp'][0])
        npt.assert_almost_equal(self.outputs['Cp_aero'], self.outputs['Cp_aero'][0])
        myCp = self.outputs['P']/(0.5*1.225*V_expect1**3.*np.pi*70**2)
        npt.assert_almost_equal(myCp, myCp[0])
        self.assertGreater(myCp[0], 0.4)
        self.assertGreater(0.5, myCp[0])
        npt.assert_almost_equal(myCp, self.outputs['Cp'])
        ...
        npt.assert_almost_equal(self.outputs['V'], V_expect1)
        npt.assert_equal(self.outputs['V_spline'], V_expect0)
        npt.assert_almost_equal(self.outputs['Omega'], Omega_expect)
        npt.assert_equal(self.outputs['pitch'][:8], 0.0 )
        ...
        myCp = self.outputs['P']/(0.5*1.225*V_expect1**3.*np.pi*70**2)
        npt.assert_almost_equal(myCp[:8], myCp[0])
        npt.assert_almost_equal(myCp[:8], self.outputs['Cp'][:8])
        ...
        npt.assert_almost_equal(self.outputs['P'][7:], 5e6, decimal=0)
        ...
        # Test min & max rpm- not implemented!
        ...
        npt.assert_equal(self.outputs['V_spline'], V_expect0)
        npt.assert_almost_equal(self.outputs['Omega'], Omega_expect)
        npt.assert_equal(self.outputs['pitch'][:8], 5.0 )
        npt.assert_array_less(0.0, np.abs(self.outputs['pitch'][8:]))
        npt.assert_equal(self.outputs['Cp'], self.outputs['Cp_aero']*0.95) 
        npt.assert_array_less(self.outputs['P'][:-1], self.outputs['P'][1:])
        npt.assert_array_less(self.outputs['Q'][:-1], self.outputs['Q'][1:])
    ...
    def testRegulationTrajectoryNoRegion3(self):
        ...
        #npt.assert_array_less(0.0, np.abs(self.outputs['pitch'][8:]))
        npt.assert_almost_equal(self.outputs['Cp'][:8], self.outputs['Cp_aero'][:8]*0.95)
        npt.assert_array_less(self.outputs['P'][:7], self.outputs['P'][1:8])
        npt.assert_almost_equal(self.outputs['P'][7:], 5e6, decimal=0)
    ...
    def testRegulationTrajectoryNEW(self):
        ...
        self.assertGreater(self.outputs['rated_pitch'], 0.0)
        myCp = self.outputs['P']/(0.5*1.225*V_expect1**3.*np.pi*70**2)
        npt.assert_almost_equal(myCp[:7], myCp[0])
        npt.assert_almost_equal(myCp[:7], self.outputs['Cp'][:7])
        ...
        npt.assert_equal(self.outputs['Cp'], self.outputs['Cp_aero']*0.95)
        npt.assert_array_less(self.outputs['P'][:7], self.outputs['P'][1:8])
        npt.assert_almost_equal(self.outputs['P'][7:], 5e6, decimal=-1)
        #npt.assert_array_less(self.outputs['Q'], self.outputs['Q'][1:])
        npt.assert_array_less(self.outputs['T'], self.outputs['T'][7]+1e-1)
        #self.assertEqual(self.outputs['rated_V'], V_expect1[-1])
        self.assertAlmostEqual(self.outputs['rated_Omega'], Omega_expect[-1])
        ...
        self.assertGreater(self.outputs['rated_pitch'], 0.0)
        myCp = self.outputs['P']/(0.5*1.225*V_expect1**3.*np.pi*70**2)
        npt.assert_almost_equal(myCp[Omega_expect == Omega_tsr], myCp[6])
        npt.assert_almost_equal(myCp[Omega_expect == Omega_tsr], self.outputs['Cp'][Omega_expect == Omega_tsr])
    ...
    def test_timing(self):
        ...
        # parameters
        self.inputs['control_Vin'] = 3.
        self.inputs['control_Vout'] = 25.
        self.inputs['control_ratedPower'] = 5e6
        self.inputs['control_minOmega'] = 6.9
        self.inputs['control_maxOmega'] = 12.1
        self.inputs['control_maxTS'] = 80.
        self.inputs['control_tsr'] = 7.01754386
        self.inputs['control_pitch'] = 0.0
        self.discrete_inputs['drivetrainType'] = 'GEARED'
        self.inputs['drivetrainEff'] = 0.95
        ...
        myobj = ra.RegulatedPowerCurve(naero=naero, n_aoa_grid=n_aoa_grid, n_Re_grid=n_Re_grid, n_pc=n_pc, n_pc_spline=n_pc,
                                       regulation_reg_II5=True, regulation_reg_III=True)
        myobj.naero = naero
        myobjGB = ra.RegulatedPowerCurve_GB(naero=naero, n_aoa_grid=n_aoa_grid, n_Re_grid=n_Re_grid, n_pc=n_pc, n_pc_spline=n_pc,
                                          regulation_reg_II5=True, regulation_reg_III=True)
        myobjGB.naero = naero
        ...
        t = time.time()
        for k in range(nrep):
            myobjGB.compute(self.inputs, outputsGB, self.discrete_inputs, self.discrete_outputs)
        print('Updated:',time.time()-t)

        for k in outputs.keys():
            print(k,outputs[k],outputsGB[k])
