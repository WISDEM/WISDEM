...
class WindTurbineOntologyPython(object):
    ...
    def write_ontology(self, wt_opt, fname_output):
        ...
        if self.modeling_options["flags"]["floating"]:
            ...
            for i in range(n_joints):
                yaml_out["joints"][i]["location"] = wt_opt["floating.location"][i, :]
            ...
            for i in range(n_members):
                ...
                if "ring_stiffeners" in istruct:
                    istruct["ring_stiffeners"]["web_height"] = wt_opt[f"floating.memgrp{idx}.ring_stiffener_web_height"]
                    istruct["ring_stiffeners"]["web_thickness"] = wt_opt[
                        f"floating.memgrp{idx}.ring_stiffener_web_thickness"
                    ]
                    istruct["ring_stiffeners"]["flange_thickness"] = wt_opt[
                        f"floating.memgrp{idx}.ring_stiffener_flange_thickness"
                    ]
                    istruct["ring_stiffeners"]["flange_width"] = wt_opt[
                        f"floating.memgrp{idx}.ring_stiffener_flange_width"
                    ]
                    istruct["ring_stiffeners"]["spacing"] = wt_opt[f"floating.memgrp{idx}.ring_stiffener_spacing"]

                if "longitudinal_stiffeners" in istruct:
                    istruct["longitudinal_stiffeners"]["web_height"] = wt_opt[
                        f"floating.memgrp{idx}.axial_stiffener_web_height"
                    ]
                    istruct["longitudinal_stiffeners"]["web_thickness"] = wt_opt[
                        f"floating.memgrp{idx}.axial_stiffener_web_thickness"
                    ]
                    istruct["longitudinal_stiffeners"]["flange_thickness"] = wt_opt[
                        f"floating.memgrp{idx}.axial_stiffener_flange_thickness"
                    ]
                    istruct["longitudinal_stiffeners"]["flange_width"] = wt_opt[
                        f"floating.memgrp{idx}.axial_stiffener_flange_width"
                    ]
                    istruct["longitudinal_stiffeners"]["spacing"] = wt_opt[
                ...
                for j in range(n_ballasts):
                    if self.modeling_options["floating"]["members"]["ballast_flag_member_" + name_member][j] == False:
                        istruct["ballasts"][j]["volume"] = wt_opt[f"floating.memgrp{idx}.ballast_volume"][j]
                ...
                if self.modeling_options["floating"]["members"]["n_axial_joints"][i] > 0:
                    for j in range(self.modeling_options["floating"]["members"]["n_axial_joints"][i]):
                        yaml_out["members"][i]["axial_joints"][j]["grid"] = wt_opt[
                            f"floating.memgrp{idx}.grid_axial_joints"
                        ][j]
