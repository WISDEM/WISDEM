...
!   gradient     of useful results: m q t
!   with respect to varying inputs: tp m precurvetip q r t rtip
!                np precurve presweep presweeptip rhub precone
!   RW status of diff variables: tp:out m:in-out precurvetip:out
!                q:in-out r:out t:in-zero rtip:out np:out precurve:out
!                presweep:out presweeptip:out rhub:out precone:out
...
! in
  INTEGER, INTENT(IN) :: n, nbdirs
  REAL(dp), DIMENSION(nbdirs), INTENT(IN) :: tb, qb, mb
  REAL(dp), DIMENSION(n), INTENT(IN) :: np, tp, r, precurve, presweep
  REAL(dp), INTENT(IN) :: precone, rhub, rtip, precurvetip, presweeptip
! out
!  REAL(dp) :: t, q, m
...
&       torque(i)+torque(i+1))*0.5_dp*qb(nd) + (thrust(i)+thrust(i+1))*&
&       0.5_dp*tb(nd)
  ...
  rb(:, :) = 0.0_dp
  DO nd=1,nbdirs
    npfullb(nd, :) = z_az*flap_momentb(nd, :) + COS(cone)*thrustb(nd, :)
