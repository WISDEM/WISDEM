...
class Manager:
    ...
    def execute_landbosse(self, project_name):
        try:
            ...
            filtered_weather_window = weather_data_user_input.loc[
                (weather_data_user_input["Season"].isin(season_construct))
                & (weather_data_user_input["Time window"] == time_construct)
            ]
            filtered_weather_window = filtered_weather_window[
                0 : (math.ceil(self.input_dict["construct_duration"] * 30 * daily_operational_hours))
            ]
            ...
            self.output_dict["actual_construction_months"] = (
                self.output_dict["siteprep_construction_months"]
                + max(
