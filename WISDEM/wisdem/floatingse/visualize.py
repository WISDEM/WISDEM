...
class Visualize(object):
    ...
    def draw_spar(self, fname='spar.png'):
        ...
        self.draw_column([0.0, 0.0], self.prob['main_freeboard'], self.prob['main.section_height'],
                           0.5*self.prob['main.outer_diameter'], self.prob['main.stiffener_spacing'])
    ...
    def draw_semi(self, fname='semi.png'):
        ...
        for k in range(ncolumn):
            self.draw_column([x[k], y[k]], self.prob['offset_freeboard'], self.prob['off.section_height'],
                               0.5*self.prob['off.outer_diameter'], self.prob['off.stiffener_spacing'])

            t_full = sectional2nodal(self.prob['off.wall_thickness'])
            self.draw_ballast([x[k], y[k]], self.prob['offset_freeboard'], self.prob['off.section_height'],
                              0.5*self.prob['off.outer_diameter']-t_full,
                              self.prob['off.permanent_ballast_height'], 0.0)

            if self.prob['off.buoyancy_tank_mass'] > 0.0:
                self.draw_buoyancy_tank([x[k], y[k]], self.prob['offset_freeboard'],
    ...
    def draw_ocean(self):
        if self.fig is None: self.init_figure()
        npts = 100
    ...
    def draw_mooring(self, mooring):
        ...
        for k in range(nlines):
            ...
            mlab.plot3d(mooring[k,:,0], mooring[k,:,1], mooring[k,:,2], color=cmoor, tube_radius=0.5*self.prob['mooring_diameter'], figure=self.fig)
    ...
    def draw_column(self, centerline, freeboard, h_section, r_nodes, spacingVec=None, ckIn=None):
        ...
        for k in range(nsection):
            ...
            while z < z_nodes[k+1]:
                ...
                mlab.plot3d(rk*np.cos(th) + centerline[0], rk*np.sin(th) + centerline[1], z*np.ones(th.shape), color=(0.5,0,0), figure=self.fig)
                z += spacingVec[k]
