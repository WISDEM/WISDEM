...
prob["main.height"] = np.sum([10.0, 5.0, 5.0, 10.0])
prob["main.s"] = np.cumsum([0.0, 10.0, 5.0, 5.0, 10.0]) / prob["main.height"]
prob["main.outer_diameter_in"] = 6.5 * np.ones(5)
prob["main.layer_thickness"] = 0.03 * np.ones((1, nsection))
...
prob["radius_to_offset_column"] = 33.333 * np.cos(np.pi / 6)
prob["off.permanent_ballast_height"] = 0.1
prob["offset_freeboard"] = 12.0
prob["off.height"] = np.sum([6.0, 0.1, 15.9, 10])
prob["off.s"] = np.cumsum([0.0, 6.0, 0.1, 15.9, 10]) / prob["off.height"]
prob["off.outer_diameter_in"] = np.array([24, 24, 12, 12, 12])
prob["off.layer_thickness"] = 0.06 * np.ones((1, nsection))
