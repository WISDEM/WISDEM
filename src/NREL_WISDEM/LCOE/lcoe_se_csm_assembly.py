...
class lcoe_se_csm_assembly(ExtendedFinancialAnalysis):
    ...
    def configure(self):
        ...
        self.connect('rotor_diameter', ['trb.rotor_diameter', 'tcc_a.rotor_diameter', 'bos_a.rotor_diameter'])
        # drivetrain
        self.connect('machine_rating', ['trb.machine_rating', 'tcc_a.machine_rating', 'bos_a.machine_rating', 'opex_a.machine_rating'])
...
def example():
    ...
    # tower.replace('wave', LinearWaves())  # no waves
    tower.replace('soil', TowerSoil())
    tower.replace('tower', TowerWithpBEAM())

    # geometry
    towerToShaft = 2.0
    towerHt = rotor.hubHt - towerToShaft  # TODO: this needs to be part of a component
    ...
    # wind
    tower.wind.Uref = 12.0  # TODO: this needs to be connected to rated speed
    tower.wind.zref = towerHt  # TODO: also connect this
    tower.wind.z0 = 0.0
    tower.wind.shearExp = 0.2  # TODO: also connect this
    ...
    # common variables
    turbine.tower_top_diameter = 3.87  # TODO: this is not connected to the tower yet

    turbine.run()


    tower.znodes = tower.tower.z  # TODO: make this an output
    ...
    print 'tower top_deflection =', tower.top_deflection
    print 'tower z_buckling =', tower.z_buckling
    print 'tower buckling =', tower.buckling
    ...
    plt.ylim([-5e-3, 5e-3])
    plt.figure()
    plt.plot(tower.stress, tower.znodes)
    plt.plot(tower.damage, tower.znodes)
    plt.show()

    lcoe.replace('trb', 'turbine')
