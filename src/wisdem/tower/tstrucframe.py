...
def formatString(n, width, precision=None, asList=False):

    stringList = ['{0:<' + str(width) + '}']
    for i in range(1, n):
        string = '{' + str(i) + ':<' + str(width)
...
class MonopileStruc(object):
    ...
    def __init__(self, z, d, t, n, top, soil, E=210e9, G=80.8e9, rho=8500.0, sigma_y=450.0e6, g=9.81):
        ...
        for i in range(0, len(n)):
            ...
            znodes = np.r_[znodes, znode[1:]]

        n = len(znodes)

        # interpolate
        dnodes = np.interp(znodes, z, d)
        tnodes = np.interp(znodes, z, t)

        d_e = 0.5*(dnodes[:-1] + dnodes[1:])
        t_e = 0.5*(tnodes[:-1] + tnodes[1:])

        # section properties
        A = pi*d_e*t_e
        r = (d_e - t_e/2)/(d_e+t_e/2)
        As = A/(0.54414 + 2.97294*r - 1.51899*r**2)
        I = pi/8*d_e**3*t_e
        J = 2*I
        ...
        reactions = np.zeros((1, 7))
        reactions[0, :] = [1, 1, 1, 1, 1, 1, 1]

        elements = np.zeros((n-1, 12))
        elements[:, 0] = np.arange(0, n-1)
        elements[:, 1] = np.arange(1, n)
        elements[:, 2] = A
        elements[:, 3] = As
        elements[:, 4] = As
        elements[:, 5] = J
        ...
        self.__writeFile(nodes, reactions, elements)
        exit()
    ...
    def __writeFile(self, nodes, reactions, elements):

        nN = nodes.shape[0]
        nR = reactions.shape[0]
        nE = elements.shape[0]


        f = open('monopile.3dd', 'w')
        f.write('Input Data file for Monopile (N, m, kg)\n')
        f.write('\n')
        f.write('\n')
        f.write('# node data\n')
        f.write('{0}\t# number of nodes\n'.format(nN))
        fstring = formatString(5, width=8, precision=8)
        f.write(fstring.format('# node', 'x-coord', 'y-coord', 'z-coord', 'radius'))
        for i in range(nN):
            f.write(fstring.format(i+1, *nodes[i, :]))

        f.write('\n')
        f.write('\n')
        f.write('# reaction data\n')
        f.write('{0}\t# number of nodes with reactions\n'.format(nR))
        fstring = formatString(7, width=4)
        f.write(fstring.format('# node', 'X', 'Y', 'Z', 'XX', 'YY', 'ZZ\t0:free, 1:fixed'))
        for i in range(nR):
            f.write(fstring.format(*map(int, reactions[i, :])))

        f.write('\n')
        f.write('\n')
        f.write('# frame element data\n')
        f.write('{0}\t# number of frame elements\n'.format(nE))
        fstring = formatString(13, width=12, precision=10, asList=True)
        fstring[0] = '{0:<5}'
        fstring[1] = '{1:<5}'
        fstring[2] = '{2:<5}'
        fstring = '\t'.join(fstring) + '\n'
        f.write(fstring.format('element', 'j1', 'j2', 'Ax', 'Asy', 'Asz', 'Jx', 'Iy', 'Iz', 'E', 'G', 'roll', 'density'))
        for i in range(nE):
            f.write(fstring.format(i+1, int(elements[i, 0])+1, int(elements[i, 1])+1, *elements[i, 2:]))
