...
class csmFinance(object):
    ...
    def compute(self, ratedpwr, tcc, opex, bos, aep, fcr, constructionrate, taxrate, discountrate, constructiontime, projlifetime, turbineNum):
        """
        ...
        self.LCOE = ((self.tcc + self.bos)* (1+self.constructionFinancingRate) * self.fixedChargeRate / self.aep) + (self.opex * (1-self.taxRate)) / self.aep                        
        ...
        self.COE = (iccKW*amortFactor) / (8760*capFact) + (self.opex/self.aep)
...
def example():
    ...
    print "Onshore"
    lcoe.compute(ratedpwr, tcc, opex, bos, aep, \
