...
class lcoe_csm_assembly(Assembly):
    ...
    # Parameters
    drivetrain_design = Enum('geared', ('geared', 'single_stage', 'multi_drive', 'pm_direct_drive'), iotype='in', group='Drivetrain')
    altitude = Float(0.0, units = 'm', iotype='in', desc= 'altitude of wind plant', group='Plant AEP')
    turbine_number = Int(100, iotype='in', desc = 'total number of wind turbines at the plant', group='Global')
    year = Int(2009, iotype='in', desc = 'year of project start', group='Plant Finance')
    month = Int(12, iotype='in', desc = 'month of project start', group='Plant Finance')
    ...
    shear_exponent = Float(0.1, iotype='in', desc= 'shear exponent for wind plant', group='Plant AEP')
    wind_speed_50m = Float(8.35, units = 'm/s', iotype='in', desc='mean annual wind speed at 50 m height', group='Plant AEP')
    weibull_k= Float(2.1, iotype='in', desc = 'weibull shape factor for annual wind speed distribution', group='Plant AEP')
    soiling_losses = Float(0.0, iotype='in', desc = 'energy losses due to blade soiling for the wind plant - average across turbines', group='Plant AEP')
    array_losses = Float(0.06, iotype='in', desc = 'energy losses due to turbine interactions - across entire plant', group='Plant AEP')
    availability = Float(0.94287630736, iotype='in', desc = 'average annual availbility of wind turbines at plant', group='Plant AEP')
    thrust_coefficient = Float(0.50, iotype='in', desc='thrust coefficient at rated power', group='Plant AEP')
    max_efficiency = Float(0.90, iotype='in', desc = 'maximum efficiency of rotor and drivetrain - at rated power', group='Plant AEP') # TODO: should come from drivetrain
    # Extra TCC parameters
    blade_number = Int(3, iotype='in', desc = 'number of rotor blades', group='Turbine Cost')
    offshore = Bool(True, iotype='in', desc = 'boolean for offshore')
    advanced_blade = Bool(False, iotype='in', desc = 'boolean for use of advanced blade curve', group='Turbine Cost')
    crane = Bool(True, iotype='in', desc = 'boolean for presence of a service crane up tower', group='Turbine Cost')
    advanced_bedplate = Int(0, iotype='in', desc= 'indicator for drivetrain bedplate design 0 - conventional', group='Turbine Cost')   
    advanced_tower = Bool(False, iotype='in', desc = 'advanced tower configuration', group='Turbine Cost')
    # Extra Finance parameters
    fixed_charge_rate = Float(0.12, iotype = 'in', desc = 'fixed charge rate for coe calculation', group='Plant Finance')
    construction_finance_rate = Float(0.00, iotype='in', desc = 'construction financing rate applied to overnight capital costs', group='Plant Finance')
    tax_rate = Float(0.4, iotype = 'in', desc = 'tax rate applied to operations', group='Plant Finance')
    discount_rate = Float(0.07, iotype = 'in', desc = 'applicable project discount rate', group='Plant Finance')
    construction_time = Float(1.0, iotype = 'in', desc = 'number of years to complete project construction', group='Plant Finance')
    project_lifetime = Float(20.0, iotype = 'in', desc = 'project lifetime for LCOE calculation', group='Plant Finance')
