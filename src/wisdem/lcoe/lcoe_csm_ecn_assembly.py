...
class lcoe_csm_ecn_assembly(Assembly):
    ...
    #OPEX outputs
    availability = Float(0.94287630736, iotype='out', desc = 'average annual availbility of wind turbines at plant') # now an opex output
    #TCC outputs
    turbine_mass = Float(0.0, units='kg', iotype='out', desc='turbine mass')
    #Finance outputs
    lcoe = Float(iotype='out', desc='_cost of energy - unlevelized')  

    def __init__(self, ssfile_1):

        self.ssfile_1 = ssfile_1
    
        super(lcoe_csm_ecn_assembly, self).__init__()
    ...
    def configure(self):
        ...
        # plant configuration
        # climate
        self.connect('sea_depth', ['bos_a.sea_depth', 'fin_a.sea_depth']) # 'opex_a.sea_depth',  # ECN only offshore
        ...
        # financial
        self.connect('year', ['tcc_a.year', 'bos_a.year']) # , 'opex_a.year' # ECN doesnt need
        self.connect('month', ['tcc_a.month', 'bos_a.month']) # , 'opex_a.month' # ECN doesnt need
        ...
        self.connect('project_lifetime',['fin_a.project_lifetime','opex_a.project_lifetime']) # new opex conx
        ...
        self.connect('opex_a.availability', 'aep_a.availability') # new opex conx
        ...
        # opex_a
        self.connect('opex_a.availability','availability')
        # tcc_a
...
def example(ssfile_1):

    lcoe = lcoe_csm_ecn_assembly(ssfile_1)
...
if __name__=="__main__":
    ...
    ssfile_1 = 'C:/Models/ECN Model/ECN O&M Model.xls' # insert your path to the ECN Model here
